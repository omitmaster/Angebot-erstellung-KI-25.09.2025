...
-- Enable RLS for handwerker_profiles
...
BEGIN
  ...
  -- If user_type is handwerker, create handwerker profile
  IF COALESCE(NEW.raw_user_meta_data ->> 'user_type', 'kunde') = 'handwerker' THEN
    ...
    VALUES (
      NEW.id,
      COALESCE(NEW.raw_user_meta_data ->> 'trade_category', 'general')
    )
    ON CONFLICT (id) DO NOTHING;
  END IF;

  -- If user_type is kunde, create kunde profile
  IF COALESCE(NEW.raw_user_meta_data ->> 'user_type', 'kunde') = 'kunde' THEN
